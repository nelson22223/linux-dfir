...
映射文件优先经 `/proc/PID/map_files/地址范围` 打开，再尝试目标进程根目录下的文件。同路径文件仍要求 maps 的设备号/inode 与打开后的属性一致；仅对真实 procfs 的直接 map_files 入口，允许使用实时映射行和已打开句柄身份交叉核实不同的文件身份视图，并保留原有进程及映射前后稳定性检查。核实失败不能按库名放行。结构化报告 `mapping_identity_views` 保存发生视图差异时的入口、路径及双方身份；失败信息保存各入口原因，避免后一次错误掩盖前一次权限错误。同一进程同一对象重复失败只记录一次。

ELF 的大型 NOBITS 节区不占用文件读取预算，未消费的压缩调试节区不再一律报错。节区名字表及检测所需动态符号、关联字符串和版本元数据仍须满足安全读取要求；不解压不受限的元数据，不放宽文件边界和整体读取上限。

网络扫描因新增或变化的 PID 产生快照缺口时最多尝试三轮。最终稳定轮次可消除此前同类临时缺口，持续变化仍为 INCONCLUSIVE；其他 I/O 错误及各轮已采到的感染证据保留，不跨轮拼接 socket 归属。

这些兼容性处理不等于已确认具体信创主机的根因：原始日志没有设备号、inode 和读取入口。Ubuntu ARM64 VM 的普通映射、OverlayFS、真实匿名共享映射及合成差异回退测试用于回归验证；不能替代实际信创系统及其 libdcf.so 的复测。相关接口依据：[proc map_files](https://man7.org/linux/man-pages/man5/proc_pid_map_files.5.html)、[OverlayFS 身份属性](https://kernel.org/doc/html/latest/filesystems/overlayfs.html)。
